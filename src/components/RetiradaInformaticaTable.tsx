...
export const RetiradaInformaticaTable = () => {
  const [situations, setSituations] = useState<RetiradaSituation[]>([]);
  const [loading, setLoading] = useState(true);
  const [searchTerm, setSearchTerm] = useState('');
  const [showDialog, setShowDialog] = useState(false);
  const [deleteItem, setDeleteItem] = useState<{ id: string; name: string } | null>(null);
  const [editingSituation, setEditingSituation] = useState<RetiradaSituation | null>(null);
  ...
  const handleEdit = (situation: RetiradaSituation) => {
